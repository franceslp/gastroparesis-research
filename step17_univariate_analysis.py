...
# PROCEDURE FLAGS
# ==========================================
flag_cols = ["flag_any_stimulator", "flag_combined_procedure",
             "flag_pre_stimulator", "flag_post_stimulator"]
...
if "metformin_baseline" in df.columns:
    df["metformin_baseline"] = df["metformin_baseline"].fillna(0).astype(int)
else:
    df["metformin_baseline"] = 0
...
if race_col in df.columns:
    df["race_simplified"] = df[race_col].fillna("Unknown").apply(
...
print("\n--- AGE GROUPS ---")
for low, high, label in age_bins:
    r_pct = 100 * ((age >= low) & (age < high)).loc[resp.index].mean()
    nr_pct = 100 * ((age >= low) & (age < high)).loc[non_resp.index].mean()

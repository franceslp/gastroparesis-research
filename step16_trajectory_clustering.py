...
# procedure_code and diabetes_type already in longitudinal
df = longitudinal.merge(
    demographics[["patient_id","sex","race","ethnicity","patient_regional_location","approximate_age"]],
...
# For each patient, define "confirmation change" as Y2 if available, else Y3
# This is the key relaxation — Y3 substitutes for Y2 when missing
...
def assign_trajectory(row):
    ...
    # Require baseline + Y1 + at least Y2 or Y3
    ...
    # Early sustained improver: improved at Y1 AND confirmed at Y2 (or Y3)
    if y1 <= -THRESHOLD and confirm_change <= -THRESHOLD:
        return "Early sustained improver"

    # Late improver: no major Y1 improvement but improved and sustained later
    if y1 > -THRESHOLD:
        # Improvement confirmed at Y2 or Y3
        if confirm_change <= -THRESHOLD:
            return "Late improver"
        # Y2 flat but Y3 and Y4 both improved
        if pd.notna(y3) and y3 <= -THRESHOLD:
            if pd.isna(y4) or y4 <= -THRESHOLD:
                return "Late improver"

    # Sustained worsener: worsened at Y1 AND confirmed at Y2/Y3
    if y1 >= THRESHOLD and confirm_change >= THRESHOLD:
        return "Worsener"

    # Partial improver: at least 2 timepoints with meaningful improvement
...
trajectory_order = ["Early sustained improver","Late improver","Partial improver","Stable non-responder","Worsener"]
